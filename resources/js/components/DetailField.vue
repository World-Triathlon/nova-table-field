    ...
    <template slot="value">
      <Table :edit-mode="false" class="overflow-hidden" v-if="theData.length > 0">
        <div class="bg-white overflow-hidden">
          <TableRow v-if="field.headings.length > 0" :row="{cells: field.headings}" :index="-1" :disabled="true" />
    ...
    this.theData = _.map(valuesArray, cells => {
      if(Array.isArray(cells)) {
        cells = cells.sort((a, b) => {
          ...
          return this.field.headings.indexOf(a) - this.field.headings.indexOf(b)
